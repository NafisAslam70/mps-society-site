...
export default function HeroSection() {
  ...
  }, [cur, slides.length, websiteData.mainPage?.hero?.images, isInitialLoad, isLoadingWebsiteData]);
    ...
    <>
      ...
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden bg-gradient-to-t from-gray-400 via-gray-500 to-white select-none">
            ...
            >
              <Image
                ...
                onError={(e) => {
                  ...
                  console.error(`Image load failed for ${s.src}`);
                  e.target.src = "/meed1.jpg"; // Reliable fallback
                }}
                style={{ animation: i === cur ? "kenBurns 7s linear infinite" : "none" }}
              />
        ...
        <motion.div
          ...
          dir={isAr ? "rtl" : "ltr"}
        >
          {websiteData.mainPage?.hero?.logo && (
            <motion.img
              src={websiteData.mainPage?.hero?.logo}
              alt="Logo"
              className="mx-auto mb-4 h-24 w-24 object-contain drop-shadow-2xl"
              onError={(e) => {
                e.target.style.display = "none";
                console.error(`Logo load failed for ${websiteData.mainPage?.hero?.logo}`);
